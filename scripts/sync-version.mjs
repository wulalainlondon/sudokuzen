...
const cachePolicyPath = path.join(repoRoot, 'src', 'pwa', 'cachePolicy.ts');
...
const dataCacheName = fs.readFileSync(cachePolicyPath, 'utf8').match(/DATA_CACHE_NAME\s*=\s*['"]([^'"]+)['"]/)[1];
const swText = swTemplate.replaceAll('__APP_VERSION__', appVersion).replaceAll('__DATA_CACHE_NAME__', dataCacheName);
