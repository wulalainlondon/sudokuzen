...
export const duoSubmitFinish = functions.https.onCall(async (data: SubmitFinishRequest, context) => {
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (currentFinishTime !== null && currentFinishTime !== undefined) {
      throw new functions.https.HttpsError('already-exists', 'Finish already submitted for this player.');
    }
...
export const duoAutoForfeitOpponent = functions.https.onCall(async (data: AutoForfeitRequest, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'Must be authenticated to forfeit opponent.');
  }
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (myFinishTime === null || myFinishTime === undefined) {
      throw new functions.https.HttpsError('failed-precondition', 'Caller has not finished yet.');
    }
    ...
    const now = Date.now();
    const heartbeatMs =
      opponentHeartbeatAtMs !== null && opponentHeartbeatAtMs !== undefined ? Number(opponentHeartbeatAtMs) : 0;
    const isStale = heartbeatMs > 0 && now - heartbeatMs > DUO_STALE_HEARTBEAT_MS;

    if (!isStale) {
      throw new functions.https.HttpsError('failed-precondition', 'Opponent heartbeat is still fresh; cannot forfeit.');
    }
...
export const duoSurrender = functions.https.onCall(async (data: SurrenderRequest, context) => {
  ...
  if (currentFinishTime !== null && currentFinishTime !== undefined) {
    throw new functions.https.HttpsError('already-exists', 'Caller has already submitted a finish time.');
  }
...
export const duoWatchdog = functions.pubsub.schedule('every 2 minutes').onRun(async (_context) => {
  const now = Date.now();
  ...
  try {
    const snap = await db.collection(DUO_ROOMS).where('status', '==', 'waiting').where('guestId', '!=', null).get();
...
export const duoToggleReady = functions.https.onCall(async (data: ToggleReadyRequest, context) => {
  if (!context.auth) {
...
export const duoSetPlaying = functions.https.onCall(async (data: SetPlayingRequest, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'Must be authenticated.');
  }
  const { roomId } = data;
...
export const duoAbortGame = functions.https.onCall(async (data: AbortGameRequest, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'Must be authenticated.');
  }
  const { roomId } = data;
...
export const duoLeaveRoom = functions.https.onCall(async (data: LeaveRoomRequest, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'Must be authenticated.');
  }
  const { roomId } = data;
...
export const duoCloseResult = functions.https.onCall(async (data: CloseResultRequest, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'Must be authenticated.');
  }
  const { roomId } = data;
...
export const duoCreateRoom = functions.https.onCall(async (data: CreateRoomRequest, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'Must be authenticated.');
  }
...
export const duoJoinRoom = functions.https.onCall(async (data: JoinRoomRequest, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'Must be authenticated.');
  }
...
export const duoThrowBomb = functions.https.onCall(async (data: ThrowBombRequest, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'Must be authenticated.');
  }
