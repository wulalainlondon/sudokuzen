...
import { formatSeconds, hashStringToSeed } from '../game/utils';
...
const REALM_ORDER = [
  ...
  '本源', '寂滅', '化神', '返虛', '合道', '渡劫', '真仙', '二昇',
  '玄仙', '太乙', '大羅', '混元', '天尊', '三昇', '神王', '帝宙', '神人',
];

const REALM_TEACH_KEY: Record<string, number> = {
  '初心': 1, '鍛骨': 2, '虛空': 3, '無我': 4, '破陣': 5,
  '空鏡': 6, '星潮': 7, '玄鏈': 8, '本源': 9, '寂滅': 10,
  '化神': 11, '返虛': 12, '合道': 13, '渡劫': 14, '真仙': 15,
  '二昇': 16, '玄仙': 17, '太乙': 18, '大羅': 19, '混元': 20,
  '天尊': 21, '三昇': 22, '神王': 23, '帝宙': 24, '神人': 25,
};
...
export function renderStageMap(): void {
  ...
  tiers.forEach(tierName => {
    ...
    node.onclick = () => {
      if (isLocked) { showFeedback(lockHint, 'error'); return; }
      enterTier(tierName);
    };
    node.innerHTML = `
...
export function renderLevelGrid(): void {
  ...
  filtered.forEach((l) => {
    ...
    if (record) {
      if (typeof record === 'number') { bestTime = record; bestStars = 1; }
      else { bestTime = record.time; bestStars = record.stars || 1; if (gs.isSpeedrunMode) submissions = record.submissions || 1; }
    }

    const item = document.createElement('div');
    item.className = `level-item ${bestTime !== null ? 'completed' : ''}${isLocked ? ' locked' : ''}`;
    item.onclick = () => {
      if (isLocked) { showFeedback(getTierUnlockMessage(l.difficultyName, unlockState), 'error'); return; }
      showPreLevelModal(l.id);
    };

    const hasRecord = bestTime !== null;
    const timeStr = hasRecord ? `${Math.floor(bestTime! / 60)}:${(bestTime! % 60).toString().padStart(2, '0')}` : '--:--';
...
export function startPoolRandom(): void {
  ...
  if (!midPoolLevels.length) { showFeedback('今日隨機題庫載入失敗，請稍後重試', 'error'); return; }
  const records = readJson<Record<string, any>>(SK.RECORDS, {});
