...
const META = {
  id: 'finned_jellyfish',
  get name() {
    return t('skills.finnedJellyfishName');
  },
  subtitle: 'Finned Jellyfish',
  sweepDirection: 'outward' as const,
};

function evaluate(selectedCells: number[], cells: CellData[]): SkillPreview {
  if (selectedCells.length < 5) return makeEmptyPreview(META, '');
  return evaluateFinnedFish(selectedCells, cells, 4, META);
}
...
export const finnedJellyfishSkill: SkillDetector = { ...META, evaluate, execute };
