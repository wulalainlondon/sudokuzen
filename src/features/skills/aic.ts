...
function evaluate(selectedCells: number[], cells: CellData[]): SkillPreview {
  ...
  if (selectedCells.length > 8) return makeEmptyPreview(META, t('skills.needSelectRange', { min: '2', max: '8' }));
  ...
  const result = findAICForHints(cells, selectedCells, 8);
  if (!result) return makeEmptyPreview(META, t('skills.noAIC'));
...
export const aicSkill: SkillDetector = { ...META, evaluate, execute };
