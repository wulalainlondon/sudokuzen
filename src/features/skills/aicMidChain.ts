...
const META = {
  id: 'aic_mid_chain',
  get name() {
    return t('skills.aicMidChainName');
  },
  subtitle: 'AIC Mid-Chain',
...
function evaluate(selectedCells: number[], cells: CellData[]): SkillPreview {
  ...
  if (selectedCells.length > 11) return makeEmptyPreview(META, t('skills.needSelectRange', { min: '3', max: '11' }));
  ...
  const result = findAICForHints(cells, selectedCells, 11);
  if (!result) return makeEmptyPreview(META, t('skills.noAICMidChain'));
...
export const aicMidChainSkill: SkillDetector = { ...META, evaluate, execute };
