...
已完成 [原盤點](code-optimization-audit-2026-09-14.md) 的六項優化及房間公開恢復修正。發行目標為 `2026.09.14-V4`；以下記錄相較 `2026.09.14-V2`（`88c4c69`）的變更與部署前驗證。
...
部署驗收另發現 Playwright 的 `waitForFunction(async ...)` 會把 Promise 當成真值而提早結束。驗證腳本已改成逐次 await 實際狀態後再輪詢，確實等待控制器啟用、更新 Worker 安裝並等待、離房後才啟用。修正後 Chromium、WebKit 及實際 V2 遷移重新通過；正式 Firebase 亦確認 33 個離線模組、normal 資料快取、真正斷網重開關卡，無頁面錯誤。此驗證修正隨 V4 發布。
